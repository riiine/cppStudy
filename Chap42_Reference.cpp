...
{
    //num1 = 10; // const 지정어가 붙어있는 변수이니 값 변경이 불가능합니다.
    //num2 = 20; // const 지정어가 붙어있는 변수이니 값 변경이 불가능합니다.
    return num1 + num2;
}

/**
만일 const 참조자가 없다면?
테스트를 위해서 AddNonConst()라는 이름의 함수를 추가해줍니다.
비-상수 매개 변수입니다.
*/
...
/** 테스트를 위해서 PrintNumConstReference라는 이름의 함수를 정의해주도록 합니다. */
void PrintNumConstReference(const int& arg)
{
    /** 매개 변수에 const지정어를 붙여서 의도치않는 변형을 방지합니다. */
    //arg = 30; // (X)
    cout << "arg : " << arg << endl;
}
...
int main()
{
    ...
    */
    ...
    intRef1 = 2;
    는 값과 주소를 모두 출력해보면 intValue1과 intRef1 모두 값이 2로 변경되고
    ...
    */
    ...
    /** 참조자의 선언방법에 대해서 알아봅니다. */
    int intValue2 = 1;
    int& intRef2 = intValue2;

    /**
    참조자를 선언하는 방법은 선언 시 &(앰퍼샌드)를 붙여야 하며
    ...
    */
    ...
    참조자는 한 번 대상을 정하면 대상을 변경하지 못합니다.
    */
    intRef5 = intValue5;
    /**
    한 번 누군가를 참조했으면 참조의 대상을 변경할 수 없습니다.
    ...
    */
    ...
    여기에서 100이라는 리터럴 상수는 임시적인 값으로서 메모리에 이름도 없이 존재했다가 사라집니다.
    ...
    */
    ...
    /** 그러면 참조자는 NULL을 참조할 수 있을까요? */

    /**
    기본적으로 NULL 참조는 불가능합니다.

    NULL은 상수값 0을 define해놓은 것이므로 NULL과 0은 같습니다.
    ...
    */
    ...
    /**
    CallByValue를 테스트 해보기 위해서 SwapByValue라는 이름의 함수를 정의해 주도록 합니다.
    CallByAddress를 테스트 해보기 위해서 SwapByAddress라는 이름의 함수를 정의해 주도록 합니다.
    CallByReference를 테스트 해보기 위해서 SwapByReference라는 이름의 함수를 정의해 주도록 합니다.
    */
    ...
    /**
    CallByValue는 매개 변수에 값을 복사해서 전달해줍니다.
    원본 변수의 값이 변경되지 않습니다.
    */
    ...
    /** 
    CallByAddress는 매개변수에 주소값을 전달해줍니다.
    원본 변수의 값이 변경됩니다.
    */
    ...
    /**
    CallByReference는 매개변수에 참조를 전달해줍니다.
    원본 변수의 값이 변경됩니다.
    함수 내용을 보니 CallByAddress보다 복잡하지 않습니다. 가독성이 있습니다.
    ...
    */
    ...
    그만큼 거대한 용량의 메모리를 복사해서 재할당 해야하기 때문에 메모리 낭비가 됩니다.
    CallByValue 방식의 큰 단점입니다.

    하지만 참조자로 매개 변수에 값을 전달하게 되면 값을 복사해서 전달해주지 않기 때문에
    메모리 낭비가 없게 됩니다.
    */
