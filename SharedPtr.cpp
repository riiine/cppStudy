...
int main()
{
    ...
    */
    ...
    대부분의 경우입니다. 여러 개의 객체에서 하나의 자원을 공유해서 사용할 때입니다.
    ...
    스마트 포인터가 100퍼센트 메모리 관리를 해주지는 않습니다.
    특정 함수에서 내가 직접 메모리 관리를 해주어야 하는 경우도 있습니다.
    */
    ...
    */
    ...
    /** 테스트를 위해서 Cat이라는 이름의 클래스를 정의해주도록 합니다.*/

    /** 문장을 추가해줍니다. */
    shared_ptr<Cat> sharedPtr1(new Cat());

    /** 생성된 sharedPtr2 객체 역시 생성된 객체 Cat을 가리킵니다. */
    ...
    */
    ...
    /**
    각기 다른 Tiger 객체를 생성해서 vector 컨테이너에 넣어주었습니다.
    shared_ptr 테스트가 되지 않습니다. 주석처리 해주세요.
    */
    ...
    */
    ...
    /**
    shared_ptr의 참조 카운터는 use_count() 함수로 할 수 있습니다.
    ...
    */
    ...
    왜냐하면 Tiger객체를 생성하기 위해서 동적 할당이 한 번 일어나야 하고
    그 다음 shared_ptr의 제어 블록 역시 동적으로 할당해야 하기 때문입니다.
    두 번의 동적 할당이 발생해야 합니다. 비효율적입니다.

    동적 할당은 상당히 비싼 연산입니다. 어차피 동적 할당을 두 번 할 것이라는 것을 알고 있다면
    아예 두 개 합친 크기로 한 번 할당하는 것이 빠릅니다.
