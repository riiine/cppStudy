...
private: // 개인적인
    ...
    자식 클래스에서 조차도 부모 클래스의 private 접근 지정자 안에 있는 멤버 변수에 접근이 안됩니다.
    다른 클래스에서는 멤버 변수에 직접적으로 접근이 불가능하게 해서 멤버 변수를 보호하는 것입니다.
    멤버 변수는 변수의 이름만 보더라도 멤버 변수인 것을 알 수 있도록 m, m_, _를 변수 이름 앞에 붙여 줍니다.
    */

    /**
    Parent1클래스는 자식도 아닌 자신만이 접근이 가능한 멤버 변수
    m_IntValue1, m_IntValue2, m_IntValue3를 가지고 있습니다.
...
public:
    ...
    이때 부모 클래스의 초기화 방법은 부모클래스명(매개변수명1, 매개변수명2, ...)으로 초기화가 가능합니다.
...
/**
protected(보호된) : 부모 클래스를 상속받는 자식 클래스에서만 접근이 가능한 접근 지정자이니다.
테스트를 위해서 Parent3, Child3라는 이름의 클래스를 정의해 줍니다.

객체 지향 언어에서 멤머 변수는 무조건 private 접근 지정자 안에 둡니다.
...
public:
    ...
    */
    ...
    /** const를 함수 끝에 붙여 주면 함수를 실행하는 동안에는 멤버 변수의 값을 바꿀 수 없도록 보장합니다. */
    int GetValueA() const
    {
        // m_IntValue1 = 50; // (X)
        return m_IntValue1;
...
public:
    ...
    {
        /**
        Parent4 클래스에 접근 지정자를 넣지 않아서 모든 변수및 함수가
...
public:
    ...
    void Print()
    {
        ...
        자식 클래스에서 접근이 가능합니다.
        부모 클래스에서 멤버 변수에 접근하기 위해서 public 지정자 안에 있는 GetAge() 라는 함수로
        멤버 변수에 접근합니다. 권장하는 방법입니다.
...
int main()
{
    ...
    */
    ...
    구조체
    주로 데이터(멤버 변수)만 넣습니다.
    C++에서는 구조체에 함수를 넣을 수 있지만 C에서는 안됩니다.
    C++에서 구조체는 클래스와 기능적으로 동일합니다.
    멤버 변수뿐 아니라 생성자와 소멸자를 비롯한 멤버 함수를 가질 수 있으며
    다른 구조체나 클래스를 상속 가능하고, 다른 클래스나 구조체를 상속 받을 수 있습니다.
    멤버들은 접근 지정자로 지정되며 멤버 활용 방법 또한 클래스와 동일합니다.
    하나 틀린점은 접근 지정자를 지정하지 않으면 디폴트로 public입니다.
    ...
    */
    ...
    하나 틀린점은 접근 지정자를 지정하지 않으면 디폴트로 private입니다.
    */
    ...
    다른 클래스의 데이터(멤버 변수), 기능(멤버 함수)를 인계해서 새로운 클래스를 정의하는 것입니다.
    자식 클래스에서는 부모 클래스로부터 상속받은 멤버 이외에 자신만의 새로운 멤버들을 추가해서 사용이 가능합니다.
    상속의 가장 큰 특징은 부모 클래스의 멤버들을 자식 클래스에서 재사용할 수 있습니다.
    ...
    */
    Child1 child1(1, 2);
    /**
    Add() 함수는 자식 클래스에서 정의가 되어 있지 않지만 부모 클래스에서 정의되어 있기 때문에
    자식 클래스에서 사용이 가능합니다.
    */
    child1.Add();
    ...
    */
    ...
    Parent3 parent3;
    /**
    변수가 가질 수 있는 최대값은 10인데, 그 이상의 값도 가질수 있습니다.
    int 타입은 -2147483648 ~ 2147483647의 범위입니다.
    parent3.m_IntValue3 = 10000000000000000;  값을 오버하니 쓰레기 값으로 채워집니다.
    */
    parent3.m_IntValue3 = 10000000000000000;
